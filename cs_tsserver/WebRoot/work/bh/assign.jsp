...
<div id="scrollContent">
	...
	<c:if test="${!empty declist}">
		...
		<tr>
			...
			<td align="center">
				<a href="javascript:void(0);" onclick="tsassign(${decAtRecord.id });" style="color:blue;">派工</a>
			</td>
		</tr>
		</c:forEach>
	</c:if>
...
//派工
function tsassign(id){
	top.Dialog.open({URL:"<%=basePath%>tsAssignAction!tsassignInput.do?id="+id,Width:500,Height:300,Title:"确定派工"});
}

function query() {
	var bTime = $("#bTime").val();
	var eTime = $("#eTime").val();	
	window.location.href = "<%=basePath%>tsAssignAction!query.do?bTime=" + bTime + "&eTime=" + eTime;
}
